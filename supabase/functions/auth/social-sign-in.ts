  ...
  try {
    ...
    switch (provider) {
      ...
      case 'apple':
        ...
        const { data: idTokenData, error: idTokenError } = await supabaseAdmin.auth.signInWithIdToken({
      ...
      case 'facebook':
        ...
        const fbGraphResponse = await fetch(`https://graph.facebook.com/me?fields=id,name,email&access_token=${token}`);
        if (!fbGraphResponse.ok) {
            console.error('Erro ao buscar dados do Facebook:', await fbGraphResponse.text());
            return new Response('Erro ao autenticar com Facebook.', { status: 400 });
        }
        const fbUserData = await fbGraphResponse.json();
        
        if (!fbUserData.email) {
            return new Response('Email não disponível no Facebook. Permissões insuficientes ou usuário não forneceu.', { status: 400 });
        }
        ...
        const { data: fbSessionData, error: fbSessionError } = await supabaseAdmin.auth.admin.createSession(userIdToUse);
    ...
    if (error) {
      ...
      return new Response(`Erro ao autenticar: ${error.message}`, { status: 500 });
    }
    ...
    return new Response(JSON.stringify({ message: error.message || 'Erro interno do servidor' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
});
